<x-app-layout>
    <div class="container mx-auto max-w-screen-xl">
        <h1 class="text-4xl font-extrabold text-gray-800 text-center">旅行計画を編集</h1>

        <form action="{{ route('plans.update', $plan->id) }}" method="POST" class="bg-white p-8 rounded-lg shadow-lg space-y-6">
            @csrf
            @method('PUT')
            <!-- タイトル -->
            <div>
                <label for="title" class="block text-lg font-semibold text-gray-700">旅行計画のタイトル</label>
                <input type="text" name="plan[title]" id="title" value="{{ old('title', $plan->title) }}" required 
            ...
            <div>
                ...
                <input type="date" name="plan[start_date]" id="date" value="{{ old('start_date', $plan->start_date->format('Y-m-d')) }}" required 
                    class="mt-2 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm p-3 transition duration-200 hover:shadow-lg">
                @error('start_date')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="time" name="plan[start_time]" id="time" value="{{ old('start_time', $plan->start_time->format('H:i')) }}" required 
                    class="mt-2 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm p-3 transition duration-200 hover:shadow-lg">
                @error('start_time')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div id="destination-container">
                @foreach($plan->destinations as $index => $destination)
                    <div class="destination-item p-4 border rounded-lg shadow-md bg-white mb-4">
                        ...
                        <select name="destinations[]" required 
                            ...
                            <option value="" disabled>目的地をお気に入りしたスポットから選択してください</option>
                            @foreach($likedSpots as $likedspot)
                                <option value="{{ $likedspot->id }}" {{ $destination->id == $likedspot->id ? 'selected' : '' }}>
                                    {{ $likedspot->name }}
                                </option>
                            @endforeach
                        </select>
