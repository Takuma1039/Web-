    ...
    <div class="container mx-auto py-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div class="col-span-2">
          @php
            $currentRank = 1;  // 現在の順位
            $previousReviewCount = null;  // 前のレビュー数を保存するための変数
            $skipCount = 0;  // 同じ順位が連続した場合にスキップするための変数
          @endphp

          @forelse($reviewranking as $index => $spot)
            @php
              // 現在のスポットのレビュー数と前のスポットのレビュー数を比較
              if ($previousReviewCount === null || $previousReviewCount !== $spot->reviews_count) {
                // 前のスポットとレビュー数が異なる場合は、順位を更新
              ...
              $previousReviewCount = $spot->reviews_count;  // 現在のレビュー数を前のレビュー数に更新
            @endphp
            <div class="p-4 bg-white rounded-lg shadow-md mb-4">
              ...
              <div class="flex mb-2">
                <div class="w-1/2 pr-2">
                  ...
                  <span class="text-sm font-medium text-gray-600" id="likeCount-{{ $spot->id }}">{{ $spot->reviews_count }} reviews</span>
                </div>
