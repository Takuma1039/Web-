...
1. **全体に共通する機能**
   ...
   - **履歴機能**: サイト上部に遷移した履歴が5件表示される(5件以上は最初の履歴から削除される)。履歴クリアボタンで履歴の削除が可能
...
5. **旅行計画周りの機能**
    ...
    - **旅行計画の投稿**: 自分が作成した旅行計画を[みんなの旅行計画]に投稿して他ユーザーと計画を共有することができる

6. **みんなの旅行計画周りの機能**
    - **いいね機能**: 他ユーザーが投稿した旅行計画に対していいねができる。いいねするとマイページに登録される
    - **旅行計画リメイク機能**: いいねした旅行計画を自分用にリメイクすることができる

## 工夫点・苦労した点
* **工夫点**: ユーザー目線で使いやすいアプリを目標に作成しました。デザインは使用しながら楽しめるデザインを意識しています。機能に関してはユーザーが一度使うだけで理解できるような機能にしています。特に、旅行計画作成や旅行計画の経路は旅行中いつでもでも手軽に作成・変更できるように設計しました。また、ゲストモードでもスポットの詳細や他のユーザーが投稿した旅行計画は見られるようにして、ログインしていなくても楽しめるようにしました。
* **苦労した点**: 経路探索機能では、1つのapiだけではすべての移動手段に対応できなかったので、移動手段別で異なるapiを使用しましたが、必要なパラメータ形式がapiごとに異なっていたので、旅行計画作成時に設定した情報を用いてパラメータを設定する際にパラメータエラーで苦しめられました。苦しめられた甲斐もあり、api周りのパラメータ設定や仕様を理解でき、結果として2つのapiを用いてすべての移動手段に対応できたのでいい経験になりました。

## 今後の展望
* **最終目標: 旅行作成から旅行中のサポートまでできるアプリにしたい**
  - 時間や料金を設定すると現在地周辺でその時間内・料金内で観光・暇つぶしできるスポットやアクティビティを提供してくれる機能を作成したい
  - 旅行計画は交通機関のより詳細な情報も提供したりなど、ユーザーに優しいかつ使いやすいものにしたい
  - ユーザーごとに旅行計画の色がでるように旅行計画内の機能や項目を増やし、自由にカスタマイズできるようにしたい
  - スポット内での楽しみ方なども紹介してくれる機能も欲しい
  - フレンド機能を作成し、チャット機能やフレンド内で旅行計画を一緒に作れるというような機能も追加したい
## ER図・ワイヤーフレームなど
![ER図](https://github.com/user-attachments/assets/233e5e73-d78a-41aa-9858-1d26fae5ba73)
